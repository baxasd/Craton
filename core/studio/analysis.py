...
def process_analysis_data(df_raw):
    ...
    numeric_cols = [
        c for c in ts_df.columns if c not in ["frame", "time_sec", "timestamp"]
    ]
    ...
    stats_df = (
        df_per_sec.drop(columns=["time_sec", "timestamp", "time_min"], errors="ignore")
        .describe()
        .T
    )
    stats_df["trend/min"] = stats_df.index.map(
        lambda x: trend_metrics.get(f"slope_{x}", 0.0)
    )
...
def create_kinematic_plot(df, x_col, y_cols, names, colors, title, show_env=False):
    ...
    for y_col, name, color in zip(y_cols, names, colors):
        ...
        if show_env:
            ...
            roll_std = (
                df[y_col].rolling(window_size, min_periods=1).std().fillna(0).values
            )
...
def render():
    ...
    with st.sidebar:
        ...
        analysis_file = st.file_uploader(
            "Select File", type=["csv", "parquet"], key="analysis_uploader"
        )

        df_analysis_raw = None
        if analysis_file is not None:
            if analysis_file.name.endswith(".parquet"):
                df_analysis_raw = pd.read_parquet(analysis_file)
            else:
                df_analysis_raw = pd.read_csv(analysis_file)

        st.subheader("Resampling")
        grouping = st.selectbox(
            "Choose an option:", ["Frames", "Seconds", "Minutes"], index=1
        )
        show_env = st.checkbox("Show Variance Envelopes", value=True)

        st.subheader("Export")
        if df_analysis_raw is not None:
            ts_df, df_per_sec, df_per_min, stats_df = process_analysis_data(
                df_analysis_raw
            )
            ...
            export_df = (
                ...
                else (df_per_sec if "Seconds" in grouping else df_per_min)
            )
            st.download_button(
    ...
    if df_analysis_raw is not None:
        ...
        with st.container(border=True):
            fig_lean = create_kinematic_plot(
                plot_df,
                x_col,
                ["lean_x", "lean_z"],
                ["Sagittal (X)", "Frontal (Z)"],
                [COLOR_RIGHT, COLOR_LEFT],
                "1. Trunk Lean Dynamics",
                show_env,
            )
