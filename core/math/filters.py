...
class PipelineProcessor:
    ...
    def remove_teleportation(df: pd.DataFrame, threshold=0.5):
        ...
        for c in x_cols:
            ...
            if not all(k in df_clean.columns for k in cols):
                continue
    ...
    def repair(df: pd.DataFrame, method="linear", limit=30):
        ...
        try:
            if method == "spline":
                ...
                df_clean[valid_cols] = df_clean[valid_cols].interpolate(
                    method="spline", order=3, limit=limit, limit_direction="both"
                )
            else:
