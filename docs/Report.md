...
Depth-sensing cameras and pose estimation frameworks are used for skeletal tracking due to their low cost and ease of deployment. [1] However, the reliability of these technologies under demanding conditions, such as physical activity performed in extreme heat, remains insufficiently explored.

Fatigue introduces additional challenges for motion capture, including increased movement, instability, and subtle posture adjustments. These factors may reduce the accuracy and robustness of vision-based skeleton tracking systems. As a result, there is a need to evaluate whether existing sensing technologies can operate effectively in such contexts.

This study focuses on the technical performance of depth camera based skeleton tracking and investigates whether the integration of mmWave radar can improve motion capture reliability during exercise in high-temperature conditions. Rather than analysing the physiological causes of fatigue, the project evaluates the effectiveness of sensing technologies when exposed to movement changes associated with thermal stress.

## **Problem Statement**

During physical activity performed in extreme heat, change in posture and motion may challenge current vision based skeleton tracking systems. Depth sensing cameras can suffer from tracking instability, occlusion sensitivity, and reduced accuracy when motion becomes irregular [2], [3]

Using an Intel RealSense depth camera as the primary sensor, this project investigates whether fusing mmWave radar data can improve motion capture performance. Radar offers complementary motion sensitive information that is less affected by visual conditions but lacks anatomical structure when used independently.

The research is guided by the following questions:

- How does extreme heat affect the reliability of depth camera based skeleton tracking during exercise?
- How does the performance of depth camera tracking compare to alternative sensing approaches under these conditions?
- Can the fusion of depth camera and mmWave radar data provide a more robust and sensitive body tracking system?
...
- Assess the suitability of a fused sensing approach for posture and movement analysis during exercise in extreme heat.
