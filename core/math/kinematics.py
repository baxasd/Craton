...
def compute_all_metrics(f: Frame) -> dict:
    ...
    return {
        ...
        "r_elb": calculate_joint_angle(
            f, "right_shoulder", "right_elbow", "right_wrist"
        ),
    }
...
def generate_analysis_report(session):
    ...
    cols = ["timestamp", "frame"] + [
        c for c in df_timeseries.columns if c not in ["timestamp", "frame"]
    ]
