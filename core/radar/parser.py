...
class RadarConfig:
    ...
    def _parse(self, file_path: str):
            ...
            if cmd == "channelCfg":
                ...
                if int(val[1]) == 0:  # Only parse profile ID 0
                    chirp = {
                        "startFreq": float(
                            val[2]
                        ),  # Carrier frequency in GHz (usually 60 or 77)
                        "idleTime": float(
                            val[3]
                        ),  # Dead time between chirps in microseconds
                        "rampEndTime": float(
                            val[5]
                        ),  # Total chirp sweep duration in microseconds
                        "freqSlope": float(
                            val[8]
                        ),  # How fast the frequency ramps in MHz/us
                        "numADCsamples": int(
                            val[10]
                        ),  # I/Q samples collected per chirp
                ...
                frame = {
                    ...
                    "numLoops": int(
                        val[3]
                    ),  # Number of times the chirp repeats = Doppler velocity bins
                    "periodicity": int(
                        val[5]
                    ),  # Frame interval in milliseconds (controls FPS)
                }
        ...
        self.numRangeBins = (
            1 if self.ADCsamples == 0 else 2 ** math.ceil(math.log2(self.ADCsamples))
        )
