...
class RecordingSession:
    ...
    def _load(self):
        ...
        for raw_bytes, ts in zip(byte_list, timestamp_list):
            ...
            mat = raw.astype(np.float32).reshape(
                self.cfg.numRangeBins, self.cfg.numLoops
            )
    ...
    def build_spectrogram(self, gate_lo_m: float, gate_hi_m: float, smooth_t: int = 2):
        ...
        sl_3d = frames_3d[:, lo_bin:hi_bin, :].max(
            axis=1
        )  # Shape becomes: (Time, Velocity)
        ...
        centroid = np.where(
            w_sum > 1e-9,
            (weights * v_axis_coarse[np.newaxis, :]).sum(axis=1) / w_sum,
            0.0,
        ).astype(np.float32)
        ...
        center_idx = nv // 2
        moving_bins_db = np.delete(
            spec_db, [center_idx - 1, center_idx, center_idx + 1], axis=1
        )
        clutter_ceiling = np.percentile(moving_bins_db, 99.0)
        spec_db[:, center_idx - 1 : center_idx + 2] = np.clip(
            spec_db[:, center_idx - 1 : center_idx + 2],
            a_min=None,
            a_max=clutter_ceiling,
        )
        ...
        v_axis_highres = np.linspace(
            -cfg.dopMax, cfg.dopMax, nv * zoom_factor, dtype=np.float32
        )
...
def extract_gait_metrics(
    spec: np.ndarray, t_axis: np.ndarray, v_axis: np.ndarray
    ...
    if len(t_axis) > 20:
        ...
        try:
            ...
            peaks, _ = find_peaks(
                filtered_sig, distance=int(fps_est / 4.0), prominence=0.4
            )
